...
void turnToOrientation(int newOrientation) {
    int difference = (newOrientation - orientation + 360) % 360;
    ...
    // Orientation is updated within turnLeft() and turnRight()
}
...
void loop() {
    ...
    if (distanceBack > maxDistance) {
        maxDistance = distanceBack;
        desiredOrientation = (orientation + 180) % 360; // Backward
    }
    if (distanceLeft > maxDistance) {
        maxDistance = distanceLeft;
        desiredOrientation = (orientation + 90) % 360; // Left
    }
    if (distanceRight > maxDistance) {
        maxDistance = distanceRight;
        desiredOrientation = (orientation + 270) % 360; // Right
    }

    // Check if the maximum distance is greater than the detection threshold
    if (maxDistance > DETECT) {
        // Turn to face the direction with the maximum distance
        turnToOrientation(desiredOrientation);
        ...
        // No available path; stop motors
        stopMotors();
        Serial.println("No available path. Robot is stopping.");
        // Optionally, you can publish a message indicating that the robot is stuck
        mqttPublish(channelID, "field5=Robot is stuck");
    }
